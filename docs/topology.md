...
4. **GCP workers**: layer `02-gcp-infra` provisions paid GCE VMs across GCP projects. Auth is multi-project Workload Identity Federation (no long-lived SA JSON keys). Empty accounts seed a default pack of **two Spot `e2-medium` workers** (OpenTofu defaults) in **`europe-west9` (Paris)** by default — closest French GCE region to Marseille. Spot uses **STOP** on preemption (not DELETE) so Talos/Omni identity survives; post-apply Omni twin cleanup is automatic. v1 is **workers only** (no control plane / etcd on GCE). **Stateful databases stay on OCI** (nodes labeled `node.stawi.org/db-eligible=false`). See [docs/gcp-lifecycle.md](gcp-lifecycle.md).
