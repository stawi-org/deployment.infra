#!/usr/bin/env bash
# Download every production/inventory/oracle/*/state.yaml from R2, remove
...
# The script is intentionally broad — every oracle state.yaml is
...
aws s3 sync "s3://cluster-tofu-state/production/inventory/oracle/" "$src/" \
  ...
  --exclude "*" --include "*/state.yaml" || true

shopt -s nullglob
any=false
for f in "$src"/*/state.yaml; do
  acct=$(basename "$(dirname "$f")")
  python3 "$(dirname "$0")/prune_stale_oci_instance_ocids.py" "$f"
  KEY="production/inventory/oracle/${acct}/state.yaml"
...
if [[ "$any" != "true" ]]; then
  echo "::notice::no oracle state.yaml files in R2 — nothing to prune"
fi
