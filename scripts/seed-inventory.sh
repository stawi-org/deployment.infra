...
#   --oci-account ACCT        OCI account key
#   --oci-node NAME           repeatable
#   --oci-list-cmd CMD        command that prints OCI instances JSON
#
# Contabo seeding was removed (fleet cut off). Active compute: OCI + GCP.
# GCP empty accounts seed default workers via OpenTofu (no seed script).
set -euo pipefail
