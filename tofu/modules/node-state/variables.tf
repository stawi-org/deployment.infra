variable "provider_name" {
  type        = string
  description = "Provider kind: oracle | onprem | gcp. 'provider' is reserved, hence the _name suffix."
  validation {
    condition     = contains(["oracle", "onprem", "gcp"], var.provider_name)
    error_message = "Invalid provider_name \"${var.provider_name}\". Must be one of: oracle, onprem, gcp."
  }
}
